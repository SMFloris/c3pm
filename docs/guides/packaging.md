...
To make the release searchable and installable by name and version, add a version manifest such as `packages/vendor/raylib/5.5.0.json` to a registry. Set `download` to the same source-object form used for direct dependencies: for example, `source: "github"`, `owner`, `repository`, `rev`, and `subdir`. Pin `rev` to a commit for a stable release and optionally include its precomputed source-tree `sha256`. The library name comes from the downloaded `.c3l/manifest.json`—do not repeat `provides` in the registry version file. Add `5.5.0` to the package's `package.json` `versions` array, and update `index.json` if its `latest` changes. Every published version needs its own version manifest.
